...
void MsCoffObj::export_data_symbol(Symbol *s)
{
    ...
    // generated: __gshared int* __imp_var = &var;
    // this is needed because we don't know if a import symbol comes from a dll or static lib, so we
    // need to manually generate it for the static lib. For the dll it is generated by the linker in
    // the import library
    ...
    Symbol *imp_s = symbol_calloc(id);
    imp_s->Salignment = -1; //default alignment

    if (s->prettyIdent)
    {
        // prepend __imp_ to the pretty identifier
        char* prettyIdent = (char*)malloc(strlen(s->prettyIdent) + 6 + 1);
        memcpy(prettyIdent, "__imp_", 6);
        memcpy(prettyIdent + 6, s->prettyIdent, strlen(s->prettyIdent) + 1);
        imp_s->prettyIdent = prettyIdent;
    }
