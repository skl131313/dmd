...
elem *getTypeInfo(Type t, IRState *irs)
{
    ...
    if (global.params.isWindows && global.params.useDll && t.vtinfo.isImportedSymbol())
    {
        return el_una(OPind, TYnptr, el_ptr(toImport(t.vtinfo)));
    }
    else
    {
        return el_ptr(toSymbol(t.vtinfo));
    }
}
    ...
    extern (C++) class ToElemVisitor : Visitor
    {
        ...
        override void visit(NewExp ne)
        {
            ...
            if (t.ty == Tclass)
            {
                ...
                {
                    elem* classinfo = null;
                    if (global.params.isWindows && global.params.useDll && cd.isImportedSymbol())
                    {
        ...
        elem *toElemCast(CastExp ce, elem *e)
        {
            ...
            if (fty == Tclass && tty == Tclass)
            {
                ...
                else
                {
                     ...
                     */
                    elem* ep = null;
                    if (global.params.isWindows && global.params.useDll && cdto.isImportedSymbol())
                    {
